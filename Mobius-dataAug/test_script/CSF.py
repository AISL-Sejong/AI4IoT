...
def do_augment(daAttr):
	...
	if daAttr['augty'] == 'rotate':
		...
		prm = daAttr['augprm']
		pipe.rotate(probability=1, max_left_rotation=int(prm['max_left_rotation']), max_right_rotation=int(prm['max_right_rotation']))
		pipe.sample(int(prm['sample']))
	else:
...
def create_target(label):
	time.sleep(5)
	for filename in glob.iglob('CSE-filesystem/output/*.*'):
		rn = str(label) + "_aug" + str(a)
		os.rename(filename, 'train/'+rn+'.jpg')
