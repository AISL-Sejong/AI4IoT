...
def deleteAccessControlPolicy(serverURL, ACPname, role):
    ACPname = addSlash(ACPname)
    header = {
        "Accept":"application/json",
        "X-M2M-RI":"3408",
        "X-M2M-Origin":role,
    }
    print(serverURL+"/Mobius"+ACPname)
    res = requests.delete(serverURL+"/Mobius"+ACPname, headers=header)
    print(res.json())
